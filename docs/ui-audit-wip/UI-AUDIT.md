...
1. Migrate `notifications.css` and `toast.css` hardcoded colors to tokens
2. Resolve transition token namespace split — pick one approach, document it
3. Move `#legal-footer` inline styles to a CSS class
4. Remove commented-out container queries in `top-bar.css`
5. Expand `view-state.css` to cover calling state properly (per ui-refactor.md)
