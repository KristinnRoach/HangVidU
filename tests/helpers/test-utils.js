...
export async function createVideoConnection(page1, page2) {
  ...
  await expect(page1.locator('#status')).toContainText('Connected', { timeout: 15000 });
  await expect(page2.locator('#status')).toContainText('Connected', { timeout: 15000 });
...
export async function loadAndSyncVideo(page1, page2, videoUrl) {
  ...
  await expect(page1.locator('#syncStatus')).not.toContainText('Waiting for video', { timeout: 10000 });
  
  // Verify page2 receives the URL
  await expect(page2.locator('#streamUrl')).toHaveValue(videoUrl, { timeout: 10000 });
...
export async function testMediaControls(page) {
  // Test mute toggle
  await page.click('#toggleMute');
  await expect(page.locator('#toggleMute')).toContainText('Unmute');
  
  await page.click('#toggleMute');
  await expect(page.locator('#toggleMute')).toContainText('Mute');
  
  // Test video toggle
  await page.click('#toggleVideo');
  await expect(page.locator('#toggleVideo')).toContainText('Turn Video On');
  
  await page.click('#toggleVideo');
  await expect(page.locator('#toggleVideo')).toContainText('Turn Video Off');
}
  ...
  try {
    ...
    if (page1) {
      try {
        ...
        if (await hangUpBtn1.isVisible({ timeout: 2000 }) && await hangUpBtn1.isEnabled({ timeout: 2000 })) {
    ...
    if (page2) {
      try {
        ...
        if (await hangUpBtn2.isVisible({ timeout: 2000 }) && await hangUpBtn2.isEnabled({ timeout: 2000 })) {
