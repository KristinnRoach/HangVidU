...
export class CallFlowSimulator {
  ...
  async simulateContactCall(callerUserId, contactUserId, originalRoomId) {
    ...
    if (!contact) {
      throw new Error(
        `Contact ${contactUserId} not found for user ${callerUserId}`,
      );
    }
    ...
    await this.triggerIncomingCallDetection(
      originalRoomId,
      callerUserId,
      contactUserId,
    );
  ...
  async saveContactToStorage(userId, contact) {
    ...
    if (user.isAuthenticated) {
      ...
      await this.firebase
        .ref(`users/${userId}/contacts/${contact.uid}`)
        .set(contact);
    } else {
  ...
  async attachContactListener(userId, roomId) {
    ...
    this.logger.logListenerAttachment(
      roomId,
      'member_join',
      user.listeningRoomIds.size,
      {
        userId,
  ...
  getCallFlowSummary(roomId) {
    ...
    return {
      ...
      callType: events.find((e) => e.type === 'INITIAL_CALL_START')
        ? 'initial'
        : 'contact',
      participants: [
        ...new Set(
          events
            .flatMap((e) => [e.data.caller, e.data.callee, e.data.userId])
            .filter(Boolean),
        ),
      ],
      timeline: events,
