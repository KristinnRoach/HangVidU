...
if (isFirefox) {
  describe.skip('Large File Transfer Integration Tests', () => {});
  console.info(
    'Skipping large file transfer test in Firefox due to ICE issues',
  );
} else {
  const MB = 1024 * 1024;
  const GB = 1024 * MB;
  ...
  function parseSize(str) {
    ...
    if (!match)
      throw new Error(
        `Invalid size format: "${str}" (use e.g. "100MB" or "1.5GB")`,
      );
  ...
  function generateTestFile(
    ...
    return new File([new Blob(chunks, { type: mimeType })], name, {
      type: mimeType,
    });
  }
  ...
  async function setupPeerConnections() {
    // Add a public STUN server for better ICE success (esp. Firefox)
    const rtcConfig = {
  ...
  describe('Large File Transfer Integration Tests', () => {
    ...
    beforeAll(async () => {
      ...
      console.log(
        `[file-transfer-large] Sizes: ${TEST_SIZES.map((t) => t.name).join(', ')}`,
      );
      console.log(
    ...
    afterAll(() => {
      ...
      if (successful.length > 0) {
        ...
        const totalSec = successful.reduce(
          (sum, r) => sum + r.transferTime / 1000,
          0,
        );
        ...
        console.log('');
        console.log(
          `  Avg speed: ${avgSpeed.toFixed(1)} MB/s (~${secPer100MB.toFixed(2)}s per 100MB)`,
        );
      }
      ...
      if (failed.length > 0) {
        ...
        console.log(
          `  Practical limit: ${lastOk ? `${lastOk.sizeName} OK` : 'none passed'}, ${firstFail.sizeName} failed`,
        );
      } else if (successful.length > 0) {
        console.log(
          `  All tested sizes passed (up to ${successful[successful.length - 1].sizeName})`,
        );
      }
      ...
      it(`should transfer ${name} file successfully`, async () => {
        ...
        try {
          ...
          const receivePromise = new Promise((resolve, reject) => {
            const timeout = setTimeout(
              () => reject(new Error('File receive timeout')),
              300000,
            );
          ...
          const progressCallback = (progress) => {
            ...
            if (pct >= lastLoggedPct + 10) {
              ...
              const speed =
                elapsed > 0 ? (transferredMB / elapsed).toFixed(1) : '...';
              console.log(
                `[${name}] ${pct}% (${transferredMB.toFixed(0)}/${sizeMB.toFixed(0)} MB) ${speed} MB/s`,
              );
            }
          };
          ...
          const sendPromise = senderController.sendFile(
            testFile,
            progressCallback,
          );
          ...
          const verify = async (offset, label) => {
            const src = new Uint8Array(
              await testFile.slice(offset, offset + 1024).arrayBuffer(),
            );
            const dst = new Uint8Array(
              await receivedFile.slice(offset, offset + 1024).arrayBuffer(),
            );
          ...
          const okMiddle =
            size > 2048 ? await verify(Math.floor(size / 2), 'Middle') : true;
          const okLast = size > 2048 ? await verify(size - 1024, 'Last') : true;
          if (!okFirst || !okMiddle || !okLast) {
            throw new Error(
              `Integrity check failed: ${result.errors[0] ?? 'mismatch'}`,
            );
          }
          ...
          if (sendProgress.length > 1) {
            ...
            for (let i = 1; i < sendProgress.length; i++) {
              intervals.push(
                sendProgress[i].timestamp - sendProgress[i - 1].timestamp,
              );
            }
            result.avgProgressInterval =
          ...
          console.log(
            `[${name}] DONE ${formatDuration(transferTime)} @ ${result.throughput.toFixed(1)} MB/s`,
          );
        } catch (error) {
          ...
          const isKnownLimit = knownBrowserErrors.some(
            (e) => error.message.includes(e) || error.name === e,
          );
