...
test.describe('Video Chat Core Flow', () => {
  test('should create and join video chat successfully', async ({ browser }) => {
    ...
    try {
      ...
      const page1VideoSrc = await page1.locator('#localVideo').getAttribute('src');
      const page2VideoSrc = await page2.locator('#localVideo').getAttribute('src');
  ...
  test('should handle media controls correctly', async ({ browser }) => {
    ...
    try {
      ...
      // Test camera switching if available
      const switchCameraBtn = page1.locator('#switchCameraBtn');
  ...
  test('should handle hang up correctly', async ({ browser }) => {
    ...
    try {
      ...
      await expect(page2.locator('#status')).toContainText('Disconnected', { timeout: 10000 });
  ...
  test('should handle connection errors gracefully', async ({ browser }) => {
    ...
    try {
      ...
      await expect(page1.locator('#status')).toContainText(/not found|error|failed/i, { timeout: 10000 });
  ...
  test('should copy share link successfully', async ({ browser }) => {
    ...
    try {
      ...
      await expect(page1.locator('#status')).toContainText('copied', { timeout: 5000 });
