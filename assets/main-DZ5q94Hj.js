...
`),o=sM(new TextEncoder().encode(r)),a=await fetch("https://gmail.googleapis.com/gmail/v1/users/me/messages/send",{method:"POST",headers:{Authorization:`Bearer ${t}`,"Content-Type":"application/json"},body:JSON.stringify({raw:o})});if(!a.ok){const l=await a.json().catch(()=>({}));throw new Error(l.error?.message||`Gmail API error: ${a.status}`)}const c=await a.json();return console.log("[GMAIL] Email sent successfully:",c.id),c}async function oM(t,e,n,i){const s={sent:0,failed:0,errors:[]};for(let r=0;r<e.length;r++){const o=e[r];try{await rM(t,o.email,n,i),s.sent++,console.log(`[GMAIL] Sent to ${o.name} (${o.email})`)}catch(a){s.failed++;const c=a&&a.message||String(a);s.errors.push({email:o.email,name:o.name,error:c}),console.error(`[GMAIL] Failed to send to ${o.name}:`,c)}r<e.length-1&&await new Promise(a=>setTimeout(a,150))}return console.log(`[GMAIL] Bulk send complete: ${s.sent} sent, ${s.failed} failed`),s}async function aM(){return new Promise(t=>{const e=document.createElement("dialog");e.classList.add("add-contact-modal"),e.innerHTML=`
      <h2>Add Contacts</h2>
  ...
  `,t.appendChild(i);const s=document.createElement("div");s.className="contacts-scroll-list";const r=document.createElement("ul");r.className="contact-list";for(const h of e){const{name:f,email:p,user:g,isAlreadySaved:_}=h,T=document.createElement("li");T.className="contact-item";let N="",$="";if(_?(N='<span class="status-badge saved">✓ Saved</span>',$=""):g?(N='<span class="status-badge on-app">On HangVidU</span>',$=`
      ...
      ${$}
    `,g&&!_){const A=T.querySelector(".invite-btn");A.addEventListener("click",async()=>{A.disabled=!0,A.textContent="Sending...";try{await ep(g.uid,g.displayName),A.textContent="✓ Sent",A.classList.add("sent")}catch(ge){console.error("[ADD CONTACT] Invite error:",ge),A.textContent="Error",A.disabled=!1}})}const y=T.querySelector(".contact-checkbox");y&&!_&&(y.checked=n.has(h),y.addEventListener("change",()=>{y.checked?n.add(h):n.delete(h),u()})),r.appendChild(T)}s.appendChild(r),t.appendChild(s);const o=i.querySelector("#select-all-checkbox");o.addEventListener("change",()=>{r.querySelectorAll(".contact-checkbox:not([disabled])").forEach(f=>{f.checked=o.checked;const p=f.getAttribute("data-email"),g=e.find(_=>_.email===p);g&&(o.checked?n.add(g):n.delete(g))}),u()});const a=document.createElement("div");a.className="bulk-actions",a.innerHTML=`
