...
This document defines the first runtime state layer for `src/shared/media-next/`.
...
Purpose: represent the current runtime state of the player domain.
...
- `loading`, `ready`, `playing`, and `paused` require `currentSourceId`
...
- `status` is the source of truth for whether playback is active
- `error` state requires `error`
...
- `attached` and `active` require `currentSourceId`
