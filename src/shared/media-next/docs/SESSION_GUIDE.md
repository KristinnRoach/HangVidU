...
This is the canonical entry point for starting or resuming work on `src/shared/media-next/` in a new context.
...
- Keep all new work isolated under `src/shared/media-next/` and the dedicated ui test pages unless a small integration change is explicitly required.
...
- Review all affected `src/shared/media-next/` docs at the end of each session and update them if needed.
- Use repo-relative Markdown links only in `src/shared/media-next/` docs.
...
- When an encountered issue or a syntax/style decision is likely to recur, add it to `src/shared/media-next/docs/solved-issues/` in a topic-oriented file such as `zod.md`, `state.md`, `func-defs.md`, `naming.md`, or `browser-runtime.md`.
...
- Keep archived session summaries under `src/shared/media-next/docs/history/`.
...
- When dead, redundant, or misleading code is encountered in `src/shared/media-next/`, simplify or remove it in the same session when practical.
...
2. Review all affected `src/shared/media-next/` docs and update them if needed.
