...
- Revisit whether storage should stay per-module or consolidate under `src/shared/storage/` only after `src/contacts/*` contracts are clearly defined and documented, and after event-driven fundamentals are documented for `appBus` / `EventEmitter` usage, boundaries, and when not to use them.
